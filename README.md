...
A TotalSales Column has been added using:

=Quantity * UnitPrice

This displays the amount of revenue produced from each order.

2. Order Detail (Concatenation)

Product Name and Region have been combined to produce a single descriptive field using:

=ProductName & " - " & Region

This will make it easier to group your reports when you add a pivot table.

3. Category Look-Up (VLOOKUP)

Product Categories have been pulled from the Product Information Sheet using:

=VLOOKUP(ProductID, ProductInfo!$A:$C, 2, FALSE)

This has enriched the dataset to allow for further analysis.

4. Bonus Eligibility (IF Statement)

Orders over ₦50,000 in TotalSales:

=IF(TotalSales>50000,"Yes","No")
